...
class Childcheckout extends StatefulWidget {
  int quantitys;
  ...
  @override
  _ChildcheckoutState createState() => _ChildcheckoutState();
}

class _ChildcheckoutState extends State<Childcheckout> {
  @override
  ...
  Future<void> addCart(
    ...
    if (response.isSuccessful) {
      if (response.body['status'] == true) {
        widget.updatecarts('nool', 'nool', 'Pilih Coupon');
        setState(() {
          widget.quantitys = quantityPost;
        });

        if (quantityPost == 0) {
          widget.removedata(widget.position);
        }
      } else {
